document.addEventListener('DOMContentLoaded', () => {
    ...
    // Función para crear la tarjeta de asistencia de un estudiante
    const createStudentAttendanceCard = (studentName) => {
        const card = document.createElement('div');
        card.className = 'student-attendance-card';
        card.innerHTML = `
            <span class="student-name">${studentName}</span>
            <div class="attendance-options" data-student-name="${studentName}">
    ...
    // Lógica para manejar la selección de asistencia (simulada)
    studentAttendanceList.addEventListener('change', (event) => {
        const radio = event.target;
        if (radio.type === 'radio') {
            const studentName = radio.name.replace('attendance-', '').replace(/-/g, ' ');
            const status = radio.value;
            console.log(`Asistencia de ${studentName} actualizada a: ${status}`);
            // Aquí se haría una llamada a la API para guardar la asistencia
        }
    });
